...
BEGIN
  ...
  FOR rec IN (
    ...
    deposit_dates AS (
      SELECT DISTINCT deposit_date AS d FROM bank_deposits
       WHERE restaurant_id = p_restaurant_id
         AND deposit_date < p_before_date
    ),
    all_dates AS (
  ...
  LOOP
    ...
    SELECT COALESCE(SUM(amount), 0)
      INTO v_deposit_effect
      FROM bank_deposits
     WHERE restaurant_id = p_restaurant_id
       AND deposit_date = rec.the_date;

    -- Cumulative Balance Chaining: include sessions, transfers AND bank deposits
    -- Carry over BOTH positive and negative balances (no capping)
    v_bargeld := v_raw_bargeld + v_transfer_effect - v_deposit_effect + v_carry_over;
